    ...
    <AuthenticatedLayout>
        ...
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fade-in-down">
                ...
                <Link :href="route('booking.create')" class="group relative inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white transition-all duration-200 bg-gray-900 font-pj rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 hover:bg-gray-700 shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                    <span class="mr-2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-5 group-hover:rotate-90 transition-transform duration-300">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                    </span>
                    จองคิวใหม่
                    <div class="absolute -inset-3 rounded-xl bg-gradient-to-r from-blue-400 to-teal-400 opacity-20 blur-lg transition duration-200 group-hover:opacity-40"></div>
                </Link>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <!-- Empty State -->
                <div v-if="bookings.length === 0" class="animate-fade-in-up flex flex-col items-center justify-center py-20 bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white/50 text-center relative overflow-hidden group">
                ...
                <div v-else class="grid gap-6">
                        ...
                        <table class="table w-full">
                            ...
                            <tbody>
                                <tr v-for="(booking, index) in bookings" :key="booking.id" 
                                    class="hover:bg-white transition-all duration-300 group border-b border-slate-50 last:border-none hover:shadow-md hover:scale-[1.01] relative z-0 hover:z-10"
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div class="font-bold text-slate-800 text-base group-hover:text-blue-600 transition-colors">{{ booking.doctor.name }}</div>
                    ...
                    <div class="md:hidden space-y-4 animate-fade-in-up">
                        <div v-for="(booking, index) in bookings" :key="booking.id" 
                             class="bg-white/80 backdrop-blur-xl p-5 rounded-3xl shadow-sm border border-slate-100 active:scale-[0.98] transition-all duration-300 hover:shadow-lg hover:-translate-y-1 relative overflow-hidden"
                             :style="{ animationDelay: `${index * 100}ms` }">
